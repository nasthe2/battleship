class SeaBattle {
  ...
  startNewGame() {
    ...
    this._pcShipsMap = this.generateRandomShipMap('pc');
    this._userShipsMap = this.generateRandomShipMap('user');
  ...
  /**
   * Создание/обновление ячеек в игровых полях
   */
  drawGamePoints() {
  ...
  /**
   * Создает или сбрасывает значения ячеек, где размещаются корабли 
   */
  ...
  /**
   * Возвращает id игровой ячейки, генерируемый на базе координат
  ...
  /**
   * Создает массив с координатами полей, из которого компьютер
  ...
  /**
   * Генерирует массив, содержащий информацию о расположении кораблей 
   */
  generateRandomShipMap(player) {
    let map = [];

    // генерация карты расположения, включающая отрицательный координаты
    ...
    while (allShipsPlaced === false) {
      ...
      if (this.isPointFree(map, xPoint, yPoint) === true) {
        if ((Math.floor(Math.random() * 2)) === 1) {
          ...
          if (this.canPutVertical(map, xPoint, yPoint, shipsConfiguration[0].pointCount, this.gameFieldBorderY.length)) {
            for (let i = 0; i < shipsConfiguration[0].pointCount; i++) {
              map[yPoint + i][xPoint] = this.CELL_WITH_SHIP;
            }
          } else if (this.canPutHorizontal(map, xPoint, yPoint, shipsConfiguration[0].pointCount, this.gameFieldBorderX.length)) {
            for (let i = 0; i < shipsConfiguration[0].pointCount; i++) {
              map[yPoint][xPoint + i] = this.CELL_WITH_SHIP;
            }
          } else {
  ...
  /**
   * Возможность вставки корабля вертикально
   */
